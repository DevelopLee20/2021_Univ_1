...
public class Main extends JFrame{
    ...
    class File_Class{ 
        ...
        // XOR로 암호화와 복호화를 진행하는 메소드
        public void XOR_En_De() throws IOException{

            // BufferedReader로 일고, PrintWriter로 쓴다.
            try(BufferedReader input = new BufferedReader(new FileReader(this.name1));
            PrintWriter output = new PrintWriter(this.name2)){

                int value;

                // 하나씩 바이트를 불러서 암호화 및 복호화
    ...
    public Main(){
        ...
        button_1.addActionListener(e ->{
            ...
            try { // 암호화 객체 생성
                File_Class file = new File_Class(name1, name2);
                file.XOR_En_De();
